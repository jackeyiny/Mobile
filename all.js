$(document).ready(function () {
    var i = 1;
    $("#btnmua").click(function(){
      alert("Đẫ mua thành công. Xin cảm ơn quý khách!!!")
    });
    $("#ggmap").click(function () {
      $("#myModal").modal();
    });
   
    $("#btn1").click(function(){
      $("#my1").modal();
    });
    $("#btn2").click(function(){
      $("#my2").modal();
    });
    $("#btn3").click(function(){
      $("#my3").modal();
    });
    $("#btn4").click(function(){
      $("#my4").modal();
    });
    $("#btn5").click(function(){
      $("#my5").modal();
    });
    $("#btn6").click(function(){
      $("#my6").modal();
    });
    $("#btn7").click(function(){
      $("#my7").modal();
    });
    $("#btn8").click(function(){
      $("#my8").modal();
    });
    $("#btn9").click(function(){
      $("#my9").modal();
    });
    $("#btn10").click(function(){
      $("#my10").modal();
    });
    $("#btn11").click(function(){
      $("#my11").modal();
    });
    $("#btn12").click(function(){
      $("#my12").modal();
    });
    $("#btn13").click(function(){
      $("#my13").modal();
    });
    $("#btn14").click(function(){
      $("#my14").modal();
    });
    $("#btn15").click(function(){
      $("#my15").modal();
    });
    $("#btn16").click(function(){
      $("#my16").modal();
    });
    $("#btn17").click(function(){
      $("#my17").modal();
    });
    $("#btn18").click(function(){
      $("#my18").modal();
    });
    $("#btn19").click(function(){
      $("#my19").modal();
    });
    $("#btn20").click(function(){
      $("#my20").modal();
    });
    $("#btn21").click(function(){
      $("#my21").modal();
    });
    $("#btn22").click(function(){
      $("#my22").modal();
    });
    $("#btn23").click(function(){
      $("#my23").modal();
    });
    $("#btn24").click(function(){
      $("#my24").modal();
    });
    $("#btn25").click(function(){
      $("#my25").modal();
    });
    $("#btn26").click(function(){
      $("#my26").modal();
    });
    $("#btn27").click(function(){
      $("#my27").modal();
    });
    $("#btn28").click(function(){
      $("#my28").modal();
    });
    $("#btn29").click(function(){
      $("#my29").modal();
    });
    $("#btn30").click(function(){
      $("#my30").modal();
    });
    $("#btn31").click(function(){
      $("#my31").modal();
    });
    $("#btn32").click(function(){
      $("#my32").modal();
    });
    $("#btn33").click(function(){
      $("#my33").modal();
    });
    $("#btn34").click(function(){
      $("#my34").modal();
    });
    $("#btn35").click(function(){
      $("#my35").modal();
    });
    $("#btn36").click(function(){
      $("#my36").modal();
    });
    $("#btn37").click(function(){
      $("#my37").modal();
    });
    $("#btn38").click(function(){
      $("#my38").modal();
    });
    $("#btn39").click(function(){
      $("#my39").modal();
    });
    $("#btn40").click(function(){
      $("#my40").modal();
    });
    $("#btn41").click(function(){
      $("#my41").modal();
    });
    $("#btn42").click(function(){
      $("#my42").modal();
    });
    $("#btn43").click(function(){
      $("#my43").modal();
    });
    $("#btn44").click(function(){
      $("#my44").modal();
    });
    $("#btn45").click(function(){
      $("#my45").modal();
    });
    $("#btn46").click(function(){
      $("#my46").modal();
    });
    $("#btn47").click(function(){
      $("#my47").modal();
    });
    $("#btn48").click(function(){
      $("#my48").modal();
    });
    $("#btn49").click(function(){
      $("#my49").modal();
    });
    $("#btn50").click(function(){
      $("#my50").modal();
    });
    $("#btn51").click(function(){
      $("#my51").modal();
    });
    $("#btn52").click(function(){
      $("#my52").modal();
    });
    $("#btn53").click(function(){
      $("#my53").modal();
    });
    $("#btn54").click(function(){
      $("#my54").modal();
    });
    $("#btn55").click(function(){
      $("#my55").modal();
    });
    $("#btn56").click(function(){
      $("#my56").modal();
    });
    $("#btn57").click(function(){
      $("#my57").modal();
    });
    $("#btn1x").click(function(){
      $("#my1").modal();
    });
    $("#btn2x").click(function(){
      $("#my2").modal();
    });
    $("#btn3x").click(function(){
      $("#my3").modal();
    });
    $("#btn4x").click(function(){
      $("#my4").modal();
    });
    $("#btn5x").click(function(){
      $("#my5").modal();
    });
    $("#btn6x").click(function(){
      $("#my6").modal();
    });
    $("#btn7x").click(function(){
      $("#my7").modal();
    });
    $("#btn8x").click(function(){
      $("#my8").modal();
    });
    $("#btn9x").click(function(){
      $("#my9").modal();
    });
    $("#btn10x").click(function(){
      $("#my10").modal();
    });
    $("#btn11x").click(function(){
      $("#my11").modal();
    });
    $("#btn12x").click(function(){
      $("#my12").modal();
    });
    $("#btn13x").click(function(){
      $("#my13").modal();
    });
    $("#btn14x").click(function(){
      $("#my14").modal();
    });
    $("#btn15x").click(function(){
      $("#my15").modal();
    });
    $("#btn16x").click(function(){
      $("#my16").modal();
    });
    $("#btn17x").click(function(){
      $("#my17").modal();
    });
    $("#btn18x").click(function(){
      $("#my18").modal();
    });
    $("#btn19x").click(function(){
      $("#my19").modal();
    });
    $("#btn20x").click(function(){
      $("#my20").modal();
    });
    $("#btn21x").click(function(){
      $("#my21").modal();
    });
    $("#btn22x").click(function(){
      $("#my22").modal();
    });
    $("#btn23x").click(function(){
      $("#my23").modal();
    });
    $("#btn24x").click(function(){
      $("#my24").modal();
    });
    $("#btn25x").click(function(){
      $("#my25").modal();
    });
    $("#btn26x").click(function(){
      $("#my26").modal();
    });
    $("#btn27x").click(function(){
      $("#my27").modal();
    });
    $("#btn28x").click(function(){
      $("#my28").modal();
    });
    $("#btn29x").click(function(){
      $("#my29").modal();
    });
    $("#btn30x").click(function(){
      $("#my30").modal();
    });
    $("#btn31x").click(function(){
      $("#my31").modal();
    });
    $("#btn32x").click(function(){
      $("#my32").modal();
    });
    $("#btn33x").click(function(){
      $("#my33").modal();
    });
    $("#btn34x").click(function(){
      $("#my34").modal();
    });
    $("#btn35x").click(function(){
      $("#my35").modal();
    });
    $("#btn36x").click(function(){
      $("#my36").modal();
    });
    $("#btn37x").click(function(){
      $("#my37").modal();
    });
    $("#btn38x").click(function(){
      $("#my38").modal();
    });
    $("#btn39x").click(function(){
      $("#my39").modal();
    });
    $("#btn40x").click(function(){
      $("#my40").modal();
    });
    $("#btn41x").click(function(){
      $("#my41").modal();
    });
    $("#btn42x").click(function(){
      $("#my42").modal();
    });
    $("#btn43x").click(function(){
      $("#my43").modal();
    });
    $("#btn44x").click(function(){
      $("#my44").modal();
    });
    $("#btn45x").click(function(){
      $("#my45").modal();
    });
    $("#btn46x").click(function(){
      $("#my46").modal();
    });
    $("#btn47x").click(function(){
      $("#my47").modal();
    });
    $("#btn48x").click(function(){
      $("#my48").modal();
    });
    $("#btn49x").click(function(){
      $("#my49").modal();
    });
    $("#btn50x").click(function(){
      $("#my50").modal();
    });
    $("#btn51x").click(function(){
      $("#my51").modal();
    });
    $("#btn52x").click(function(){
      $("#my52").modal();
    });
    $("#btn53x").click(function(){
      $("#my53").modal();
    });
    $("#btn54x").click(function(){
      $("#my54").modal();
    });
    $("#btn55x").click(function(){
      $("#my55").modal();
    });
    $("#btn56x").click(function(){
      $("#my56").modal();
    });
    $("#btn57x").click(function(){
      $("#my57").modal();
    });
    
    
    $("#btnip").click(function () {
      $("#all").hide();
      $("#all1").hide();
      $("#all2").hide();
      $("#all3").hide();
      $("#ip1").show();
      $("#ip2").show();
      $("#ss1").hide();
      $("#ss2").hide();
      $("#oppo1").hide();
      $("#oppo2").hide();
      $("#vivo1").hide();
      $("#vivo2").hide();
      $("#xiaomi1").hide();
      $("#xiaomi2").hide();
      $("#realme1").hide();
      $("#realme2").hide();
      $("#nokia1").hide();
      $("#nokia2").hide();
      $("#itel").hide();
     
    });
    $("#btnss").click(function () {
      $("#all").hide();
      $("#all1").hide();
      $("#all2").hide();
      $("#all3").hide();
      $("#ip1").hide();
      $("#ip2").hide();
      $("#ss1").show();
      $("#ss2").show();
      $("#oppo1").hide();
      $("#oppo2").hide();
      $("#vivo1").hide();
      $("#vivo2").hide();
      $("#xiaomi1").hide();
      $("#xiaomi2").hide();
      $("#realme1").hide();
      $("#realme2").hide();
      $("#nokia1").hide();
      $("#nokia2").hide();
      $("#itel").hide();
     
    });
    $("#btnoppo").click(function () {
      $("#all").hide();
      $("#all1").hide();
      $("#all2").hide();
      $("#all3").hide();
      $("#ip1").hide();
      $("#ip2").hide();
      $("#ss1").hide();
      $("#ss2").hide();
      $("#oppo1").show();
      $("#oppo2").show();
      $("#vivo1").hide();
      $("#vivo2").hide();
      $("#xiaomi1").hide();
      $("#xiaomi2").hide();
      $("#realme1").hide();
      $("#realme2").hide();
      $("#nokia1").hide();
      $("#nokia2").hide();
      $("#itel").hide();
     
    });
    $("#btnvivo").click(function () {
      $("#all").hide();
      $("#all1").hide();
      $("#all2").hide();
      $("#all3").hide();
      $("#ip1").hide();
      $("#ip2").hide();
      $("#ss1").hide();
      $("#ss2").hide();
      $("#oppo1").hide();
      $("#oppo2").hide();
      $("#vivo1").show();
      $("#vivo2").show();
      $("#xiaomi1").hide();
      $("#xiaomi2").hide();
      $("#realme1").hide();
      $("#realme2").hide();
      $("#nokia1").hide();
      $("#nokia2").hide();
      $("#itel").hide();
     
    });
    $("#btnxiaomi").click(function () {
      $("#all").hide();
      $("#all1").hide();
      $("#all2").hide();
      $("#all3").hide();
      $("#ip1").hide();
      $("#ip2").hide();
      $("#ss1").hide();
      $("#ss2").hide();
      $("#oppo1").hide();
      $("#oppo2").hide();
      $("#vivo1").hide();
      $("#vivo2").hide();
      $("#xiaomi1").show();
      $("#xiaomi2").show();
      $("#realme1").hide();
      $("#realme2").hide();
      $("#nokia1").hide();
      $("#nokia2").hide();
      $("#itel").hide();
     
    });
    $("#btnrealme").click(function () {
      $("#all").hide();
      $("#all1").hide();
      $("#all2").hide();
      $("#all3").hide();
      $("#ip1").hide();
      $("#ip2").hide();
      $("#ss1").hide();
      $("#ss2").hide();
      $("#oppo1").hide();
      $("#oppo2").hide();
      $("#vivo1").hide();
      $("#vivo2").hide();
      $("#xiaomi1").hide();
      $("#xiaomi2").hide();
      $("#realme1").show();
      $("#realme2").show();
      $("#nokia1").hide();
      $("#nokia2").hide();
      $("#itel").hide();
     
    });
    $("#btnnokia").click(function () {
      $("#all").hide();
      $("#all1").hide();
      $("#all2").hide();
      $("#all3").hide();
      $("#ip1").hide();
      $("#ip2").hide();
      $("#ss1").hide();
      $("#ss2").hide();
      $("#oppo1").hide();
      $("#oppo2").hide();
      $("#vivo1").hide();
      $("#vivo2").hide();
      $("#xiaomi1").hide();
      $("#xiaomi2").hide();
      $("#realme1").hide();
      $("#realme2").hide();
      $("#nokia1").show();
      $("#nokia2").show();
      $("#itel").hide();
     
    });
    $("#btnitel").click(function () {
      $("#all").hide();
      $("#all1").hide();
      $("#all2").hide();
      $("#all3").hide();
      $("#ip1").hide();
      $("#ip2").hide();
      $("#ss1").hide();
      $("#ss2").hide();
      $("#oppo1").hide();
      $("#oppo2").hide();
      $("#vivo1").hide();
      $("#vivo2").hide();
      $("#xiaomi1").hide();
      $("#xiaomi2").hide();
      $("#realme1").hide();
      $("#realme2").hide();
      $("#nokia1").hide();
      $("#nokia2").hide();
      $("#itel").show();
     
    });
    $("#btnall").click(function () {
      $("#all").show();
      $("#all1").show();
      $("#all2").show();
      $("#all3").show();
      $("#ip1").show();
      $("#ip2").show();
      $("#ss1").show();
      $("#ss2").show();
      $("#oppo1").show();
      $("#oppo2").show();
      $("#vivo1").show();
      $("#vivo2").show();
      $("#xiaomi1").show();
      $("#xiaomi2").show();
      $("#realme1").show();
      $("#realme2").show();
      $("#nokia1").show();
      $("#nokia2").show();
      $("#itel").show();
     
    });
    $("#ctnew1").hide();
    $("#ctnew2").hide();
    $("#ctnew3").hide();
    $("#ctnew4").hide();
    $("#btnnew1").click(function(){
      $("#tintuc").hide();
      $("#ctnew1").show();
      $("#ctnew2").hide();
      $("#ctnew3").hide();
      $("#ctnew4").hide();
    });
    $("#btnnew2").click(function(){
      $("#tintuc").hide();
      $("#ctnew1").hide();
      $("#ctnew2").show();
      $("#ctnew3").hide();
      $("#ctnew4").hide();
    });
    $("#btnnew3").click(function(){
      $("#tintuc").hide();
      $("#ctnew1").hide();
      $("#ctnew2").hide();
      $("#ctnew3").show();
      $("#ctnew4").hide();
    });
    $("#btnnew4").click(function(){
      $("#tintuc").hide();
      $("#ctnew1").hide();
      $("#ctnew2").hide();
      $("#ctnew3").hide();
      $("#ctnew4").show();
    });
  $("#capnhat").hide();
  function KTDN() {
    let mauKT = /^[A-Za-z0-9]{6,16}$/;
    if ($("#txtDN").val() == "") {
      $("#tbDN").html("Không để trống");
      return false;
    }
    if (!mauKT.test($("#txtDN").val())) {
      $("#tbDN").html("* Chỉ được chữ và số, từ 6-16 kí tự");
      return false;
    }
    $("#tbName").html("*");
    return true;
  }
  $("#txtDN").blur(KTDN);
  function KTMK() {
    let mauKT = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/;
    if ($("#txtMK").val() == "") {
      $("#tbMK").html("Không để trống");
      return false;
    }
    if (!mauKT.test($("#txtMK").val())) {
      $("#tbMK").html("* Tối thiểu tám ký tự, ít nhất một chữ cái viết hoa, một chữ cái viết thường và một số");
      return false;
    }
    $("#tbMK").html("*");
    return true;
  }
  $("#txtMK").blur(KTMK);
  function KTXNMK(){
    if($("#txtKTMK").val() !=$("#txtMK").val()) {
        $("#tbKTMK").html( "* phải giống ô mật khẩu");
        return false;
    } else{
      $("#tbKTMK").html("*");
        return true;
    } 
  }
  $("#capnhat").hide();
  $("#cntc").hide();
    $("#txtKTMK").blur(KTXNMK); 
    $("#btndk").click(function () {
      if (
        KTDN()==true &&
        KTMK()==true&&
        KTXNMK()==true
      ) {
        $("#dk").hide();
        $("#capnhat").show();
      }
    });
    function kiemTraTen() {
     
      let mauKT = /^([A-Z]{1}[a-z]+)((\s{1}[A-Z]{1}[a-z]+){1,})$/;
      if ($("#txtTen").val() == "") {
        $("#tbTen").html("Không để trống");
        return false;
      }
      if (!mauKT.test($("#txtTen").val())) {
        $("#tbTen").html("* Mỗi ký tự đầu viết hoa không sử dụng số");
        return false;
      }
      $("#tbTen").html("*");
      return true;
    }
    $("#txtTen").blur(kiemTraTen);
    function KTCMND() {
      
      let mauKT = /^\d*$/;
      if ($("#txtCMND").val() == "") {
        $("#tbCMND").html("Không để trống");
        return false;
      }
      if (!mauKT.test($("#txtCMND").val())) {
        $("#tbCMND").html("* Chỉ được nhập số");
        return false;
      }
      $("#tbCMND").html("*");
      return true;
    }
    $("#txtCMND").blur(KTCMND);
    function kiemTraSDT() {
      var mauKT = /^0\d{9}$/;
      if ($("#txtSDT").val() == "") {
        $("#tbSDT").html("Không để trống");
        return false;
      }
      if (!mauKT.test($("#txtSDT").val())) {
        $("#tbSDT").html("Theo dạng 0xxxxxxxxx trong đó x là số");
        return false;
      }
      $("#tbSDT").html("*");
      return true;
    }
    $("#txtSDT").blur(kiemTraSDT);
    $("#btncn").click(function () {
      if (
        kiemTraTen()==true &&
        KTCMND()==true&&
        kiemTraSDT()==true
      ) {
        $("#capnhat").hide();
        $("#cntc").show();
        
      }
    });
    $("#tt").hide();
    $("#btndn").click(function () {
      if (
        KTDN()==true &&
        KTMK()==true
      ) {
        $("#dn").hide();
        $("#tt").show();
        
      }
    });
    function KTTD() {
      if ($("#txtTD").val() == "") {
        $("#tbTD").html("Không để trống");
        return false;
      }else{
        $("#tbTD").html("*");
        return true;
      }
      
    }
    $("#txtTD").blur(KTTD);
    function KTND() {
      if ($("#txtND").val() == "") {
        $("#tbND").html("Không để trống");
        return false;
      }else{
        $("#tbND").html("*");
        return true;
      }
      
    }
    $("#txtND").blur(KTND);
    $("#btngui").click(function () {
      if (
        kiemTraTen()==true &&
        KTTD()==true &&
        kiemTraSDT()==true &&
        KTND()==true
      ) {
        alert("Cảm ơn quý khánh hàng đã góp ý. Chúng tôi sẽ giải quyết nhanh nhất")
        
      }
    });
    

});
